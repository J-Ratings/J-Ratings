...
<head>
  <meta charset="UTF-8" />
  <title>Chess ELO Chart with Search</title>
...
<body>
  ...
  <script>
    // Once the CSV is loaded, we build all traces, then store player names in window.playerNames
    Plotly.d3.csv("plot_data.csv", function(err, rows) {
      ...
      // 2) (optional) group players by their latest ELO if you still need playerBands elsewhere
      const playerBands = {
        "<1600": [], "1600-1800": [], "1800-2000": [], "2000-2200": [], ">2200": []
      };
      players.forEach(name => {
        const dataForThis = rows.filter(r => r.Name === name);
        const lastELO = +dataForThis[dataForThis.length - 1].ELO_smooth;
        if (lastELO > 2200) {
      ...
      // 3) custom colour palette (unchanged)
      const customColours = [
      ...
      // 4) build one trace per player
      const traces = players.map((name, i) => {
        const pData = rows.filter(r => r.Name === name);
        return {
          x: pData.map(r => r.Date),
          y: pData.map(r => +r.ELO_smooth),
          mode: 'lines',
          name: name,
          line: { color: customColours[i % customColours.length] },
          hovertemplate:
            'Date: %{x|%Y-%m-%d}<br>ELO: %{y:.0f}<extra>%{fullData.name}</extra>'
        };
      });

      // 5) layout, with years on x-axis only
      const layout = {
        xaxis: {
          title: 'Date',
          type: 'date',
        ...
        margin: { t: 50 },
        shapes: [
          // vertical dashed lines every January & July
      ...
      // 6) draw the plot, then hide all traces initially
      Plotly.newPlot('plot', traces, layout, { responsive: true })
        .then(() => {
          const allIndices = traces.map((_, i) => i);
          Plotly.restyle('plot', { visible: 'legendonly' }, allIndices);
        });
    });

    // --------------------------------------------------------
    // filterPlayers(): called whenever the user types in #nameSearch
    // --------------------------------------------------------
    function filterPlayers() {
      const input = document.getElementById("nameSearch").value.toLowerCase();
      const plotDiv = document.getElementById("plot");
      const allNames = plotDiv.data.map(trace => trace.name);

      // build an array of visibility flags: true if name includes input, else 'legendonly'
      const visArray = allNames.map(name =>
        name.toLowerCase().includes(input) ? true : 'legendonly'
      );
      Plotly.restyle('plot', { visible: visArray },
        [...Array(allNames.length).keys()]
      );
    }

    // (Optional) keep your existing helper functions if you still need showAllPlayers(), hideAllPlayers(), showBand():
